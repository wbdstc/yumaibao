...
class IFCConversionService {
    ...
    // 检查IfcOpenShell是否可用
    isIfcOpenShellAvailable() {
        try {
            // 检查IfcConvert可执行文件是否存在
            const ifcConvertPath = this.getIfcConvertPath();
            return fs_1.default.existsSync(ifcConvertPath);
        }
        catch (error) {
    ...
    getIfcConvertPath() {
        ...
        return path_1.default.join(this.ifcOpenShellPath, `IfcConvert${ext}`);
    }
    // 执行IFC转换
    async convertIFC(params) {
        const startTime = Date.now();
        const logPrefix = `[IFC转换] ${path_1.default.basename(params.inputFile)}`;
        try {
            console.log(`${logPrefix} 开始转换，参数:`, params);
            // 检查输入文件是否存在
            if (!fs_1.default.existsSync(params.inputFile)) {
            ...
            try {
                originalStats = fs_1.default.statSync(params.inputFile);
                convertedStats = fs_1.default.statSync(outputFile);
                console.log(`${logPrefix} 文件大小 - 原始: ${originalStats.size}字节，转换后: ${convertedStats.size}字节`);
            }
            catch (statError) {
                console.error(`${logPrefix} 获取文件大小失败: ${statError.message}`);
                return {
                    success: false,
            ...
            try {
                ...
                console.error(`${logPrefix} 文件上传失败: ${uploadError.message}`);
                return {
                    success: false,
            ...
            try {
                ...
                console.error(`${logPrefix} 临时文件清理失败: ${cleanupError.message}`);
                // 清理失败不影响转换结果，继续执行
            }
    ...
    // 生成输出文件路径
    generateOutputFilePath(inputFile, outputFormat) {
        const baseName = path_1.default.basename(inputFile, path_1.default.extname(inputFile));
...
exports.default = new IFCConversionService();
