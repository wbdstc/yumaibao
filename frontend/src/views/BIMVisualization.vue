...
// 根据楼层生成带楼高的随机位置
const getRandomPosition = (floorId = null) => {
  // 获取楼层高度
  let floorHeight = 0
  if (floorId && floors.value.length > 0) {
    const floor = floors.value.find(f => f.id === floorId)
    if (floor) {
      // 使用楼层的高度信息，如果有的话
      floorHeight = floor.height || floor.elevation || 0
      // 如果没有直接的高度信息，根据楼层序号估算（每层3米）
      if (floorHeight === 0) {
        const floorIndex = floors.value.indexOf(floor)
        floorHeight = floorIndex * 3 // 每层楼高3米
      }
    }
  }
...
const highlightInBimViewer = (embeddedPart) => {
  ...
  // 脉冲动画效果 - 增强版，更明显的视觉效果
  let pulseTime = 0
  const pulseDuration = 5000 // 脉冲持续5秒
  ...
  const pulseAnimation = () => {
    ...
    if (elapsed > pulseDuration) {
      // 动画结束，保持高亮状态
      targetObject.scale.set(1.2, 1.2, 1.2) // 保持略微放大
      targetObject.material.emissiveIntensity = 0.8
      return
    }
    
    pulseTime += 0.08 // 加快动画速度
    
    // 缩放脉冲（更明显的呼吸效果）- 从1.0到2.0之间缩放
    ...
    // 颜色闪烁效果（黄色和白色之间）
    const colorIntensity = 0.5 + Math.sin(pulseTime * 4) * 0.5
    targetObject.material.emissive.setHSL(0.15, 1.0, colorIntensity * 0.5)
